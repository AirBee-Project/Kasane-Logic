...
- **`temporal_id`**
  ...
  - **用途**: 時間を持たない（純粋な3D/2Dの）空間のみを扱う場合は不要です。OFFにすると各IDから時間フィールドそのものが消え（`FlexId` 24→16バイト、`SingleId` 32→16バイト、`RangeId` 56→28バイト）、木の分割軸も F/X/Y の3軸に戻るため、時間軸ぶんのメモリと計算のオーバーヘッドが消えます。
  - **注意**: OFFのビルドで時間付きのIDを作ろうとすると（`with_time()` / `FlexId::new_with_time()` / 時間付き文字列のパース / 時間付き JSON の読み込み）エラーになります。黙って全時間へ丸めることはしません。

#### デフォルトで無効な機能

- **`persist`**
  - **機能**: `rkyv` によるバイト列への永続化（`SpatialIdMap::to_bytes` / `from_bytes`）を有効化します。
  - **注意**: 保存形式は `temporal_id` の有無で異なります（4軸=バージョン2 / 3軸=バージョン3）。異なる構成で書いたファイルは読み込み時に弾かれます。また、時間軸の導入前に書かれたバージョン1のファイルには**移行パスがなく**、読み込めません（作り直しが必要です）。
