...
- **`serde`**
  - **機能**: `serde` および `serde_json` クレートを使用し、各種構造体・列挙型のシリアライズ／デシリアライズを提供します。また、`SpatilaIdSet` や `SpatilaIdTable` といったコレクションに `to_json()` メソッドが実装され、JSONフォーマットに直接書き出すことが可能になります。
