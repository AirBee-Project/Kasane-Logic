...
#[cfg(any(test))]
use crate::{RangeId, SingleId, SpatialIdSet, SpatialIds, VBitSet};
...
pub fn set_a() -> VBitSet {
    ...
    set.insert(RangeId::new(5, [-7, 11], [1, 5], [5, 30]).unwrap());
    set.insert(RangeId::new(3, [2, 2], [1, 5], [2, 2]).unwrap());
    set
}
...
pub fn set_b() -> VBitSet {
    let mut set = VBitSet::default();
    set.insert(RangeId::new(4, [5, 4], [4, 5], [9, 10]).unwrap());
    set.insert(SingleId::new(2, 2, 2, 2).unwrap());
    set
}
...
pub fn set_c() -> VBitSet {
    let mut set = VBitSet::default();
    set.insert(SingleId::new(2, 1, 1, 1).unwrap());
    set.insert(SingleId::new(3, 4, 4, 4).unwrap());
    set.insert(RangeId::new(4, [-7, 11], [4, 10], [1, 9]).unwrap());
    set
}

/// テストのために、ランダムなSetを生成する戦略
#[cfg(any(test))]
