...
mod tests {
    ...
    fn first_insert_range_id_largest() {
        ...
        set.insert(&range_id, &"kizi".to_string());
    ...
    fn first_insert_single_id_smallest_edge_start() {
        ...
        set.insert(&single_id, &"cat".to_string());
        ...
        assert_eq!(
            (RangeId::from(single_id), &"cat".to_string()),
            range_ids.first().unwrap().clone()
        )
    }
    ...
    fn first_insert_single_id_smallest_edge_end() {
        ...
        .unwrap();
        set.insert(&single_id, &"aaaaaaa".to_string());
        ...
        //含まれるIDは生成したSingleIdと一致するはず
        assert_eq!(
            (RangeId::from(single_id), &"aaaaaaa".to_string()),
            range_ids.first().unwrap().clone()
        )
    }
    ...
    fn multiple_insert_single_id_overlap() {
        ...
        set.insert(&single_id_a, &"neko".to_string());
        set.insert(&single_id_b, &"inu".to_string());
        ...
        //含まれるIDは生成したSingleIdと一致するはず
        assert_eq!(
            (RangeId::from(single_id_b), &"inu".to_string()),
            range_ids.first().unwrap().clone()
        )
    }
    ...
    fn multiple_insert_single_id_join() {
        ...
        set.insert(&single_id_a, &"inu".to_string());
        set.insert(&single_id_b, &"inu".to_string());
    ...
    ///2つのIDを挿入するテスト
    ///隣のIDでも値が異なれば、くっつかないことを検証
    #[test]
    fn multiple_insert_single_id_not_join() {
        //Setの新規作成
        let mut set = TableOnMemory::new();
        ...
        set.insert(&single_id_a, &"inu".to_string());
        set.insert(&single_id_b, &"neko".to_string());
        ...
        assert_eq!(
            *range_ids.first().unwrap(),
            (
                RangeId::new(4, [3, 3], [2, 2], [1, 1]).unwrap(),
                &"inu".to_string()
            )
        );
        ...
        assert_eq!(
            *range_ids.index(1),
            (
                RangeId::new(4, [3, 3], [2, 2], [0, 0]).unwrap(),
                &"neko".to_string()
            )
        )
    }
    ...
    fn first_insert_range_id_join() {
        ...
        assert_eq!(
            *range_ids.first().unwrap(),
            (RangeId::new(0, [0, 0], [0, 0], [0, 0]).unwrap(), &0u64)
        )
    }
}
