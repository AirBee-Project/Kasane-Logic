...
use crate::{
    ...
    spatial_id::{BlockSegmentation, helpers::fast_intersect},
};
...
pub trait Scanner: Sized {
    ...
    ///[FlexIdScanPlan]型を返す
    fn flex_id_scan_plan<T: BlockSegmentation>(&'_ self, target: T) -> FlexIdScanPlan<'_> {
...
impl<'a> FlexIdScanPlan<'a> {
    pub fn new<T: BlockSegmentation, S: Scanner>(scanner: &'a S, target: T) -> Self {
