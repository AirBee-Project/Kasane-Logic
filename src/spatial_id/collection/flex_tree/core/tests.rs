...
mod merge_with_tests {
    ...
    /// 重なるSegmentでは resolve(a, b) で合成し、片側だけのSegmentはそのまま残る。
    #[test]
    ...
    /// 粗いSegment（a）と、その内部の細かいSegment（b）を重ねると、重なった部分Segmentだけが
    /// resolve され、残りの部分Segmentは a の値を保つ（降下してSegment単位で解決される）。
    #[test]
    fn merge_with_resolves_coarse_against_fine() {
        // z=18 の 1 Segmentは z=20 では F/X/Y 各 4 分割 = 4x4x4 = 64 の部分Segmentに展開される。
        let mut a = FlexTreeCore::new();
