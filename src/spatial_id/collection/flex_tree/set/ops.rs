...
use crate::spatial_id::collection::flex_tree::core::node_ops::{
    TSetDifference, TSetIntersection, TSetUnion,
};
...
impl BitOr<&SpatialIdSet> for &SpatialIdSet {
    ...
    fn bitor(self, rhs: &SpatialIdSet) -> Self::Output {
        let shard = FlexTreeCore::shard_for_union(&self.inner, &rhs.inner);
        SpatialIdSet {
...
impl BitAnd<&SpatialIdSet> for &SpatialIdSet {
    ...
    fn bitand(self, rhs: &SpatialIdSet) -> Self::Output {
        let shard = FlexTreeCore::shard_for_intersection(&self.inner, &rhs.inner);
        SpatialIdSet {
            inner: self
                .inner
