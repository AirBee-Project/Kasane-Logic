...
//!
//!    要約（`MapArena::summary`）は `leaf_count` / `split_mask` を焼き込むが、
//!    `ArenaSummary` のフィールドは固定なので、`Node` に**新しい**キャッシュを足しても
//!    バイト列は変わらない。この性質は `ArenaSummary` を触ったときだけ破れる。
//!
...
//! - `MapArena` / `ArenaNode` / `ArenaSummary` を変えたなら → **想定どおり**。形式バージョンを
//!   上げ、移行方針を決めてからスナップショットを更新する。ただし
//!   **値を足したいだけならまず `MapArena::ext`（TLV）を検討すること**。あちらは
//!   レイアウトが変わらないのでバージョンを上げずに済み、既に書き出した KVS 上の
//!   シャードを書き直さなくてよい
