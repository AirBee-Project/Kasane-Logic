...
pub(crate) fn grid_op<V, P>(axis: GridAxis, z: ZoomLevel, radius: u32) -> Option<GridOp<V>>
where
    ...
    P: MergePolicy<V> + Send + Sync + 'static,
{
    ...
    Some(GridOp::falloff(
        ...
        z,
        radius,
        Box::new(move |value: &V, distance: u32| attenuate(value, distance, radius)),
...
/// （[`FlexId::falloff_linear_x`](crate::FlexId::falloff_linear_x) 等）と、一様ズームの
/// 平坦表現をレーン走査する経路
...
/// 呼び出し側が先に弾いている。
pub(crate) fn attenuate<V>(value: &V, distance: u32, radius: u32) -> V
where
