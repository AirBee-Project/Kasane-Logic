use super::f::{FShift, FShiftCustomParameter};
use super::x::{XShift, XShiftCustomParameter};
use super::y::{YShift, YShiftCustomParameter};
use crate::{F_MAX, FlexId, SingleId, SpatialIdTable, UnaryOperator};
...
#[test]
fn f_shift_up_moves_cell() {
    let table = table_with(25, 0, 100, 100);
    let result =
        FShift::execution(&table, FShiftCustomParameter::Index { z: 25, index: 3 }).unwrap();
...
#[test]
fn f_shift_down_moves_cell() {
    let table = table_with(25, 10, 100, 100);
    let result =
        FShift::execution(&table, FShiftCustomParameter::Index { z: 25, index: -4 }).unwrap();
...
#[test]
fn f_shift_out_of_range_is_error() {
    // z=25 の最上セルをさらに上へ動かすと範囲外。
    let table = table_with(25, F_MAX[25], 100, 100);
    let result = FShift::execution(&table, FShiftCustomParameter::Index { z: 25, index: 1 });
...
#[test]
fn x_shift_moves_cell() {
    let table = table_with(25, 0, 100, 100);
    let result =
        XShift::execution(&table, XShiftCustomParameter::Index { z: 25, index: 5 }).unwrap();
...
#[test]
fn x_shift_wraps_around_seam() {
    // z=2 の最東セル x=3 を +1 すると、経度の境界を越えて x=0 へ巡回する。
    let table = table_with(2, 0, 3, 0);
    let result =
        XShift::execution(&table, XShiftCustomParameter::Index { z: 2, index: 1 }).unwrap();
...
#[test]
fn x_shift_full_circle_returns_to_origin() {
    // z=2 では一周が4セル。+4 で元の位置へ戻る。
    let table = table_with(2, 0, 1, 0);
    let result =
        XShift::execution(&table, XShiftCustomParameter::Index { z: 2, index: 4 }).unwrap();

    assert!(present(&result, 2, 0, 1, 0));
}

#[test]
fn x_shift_splits_when_crossing_seam() {
...
#[test]
fn y_shift_moves_cell() {
    let table = table_with(25, 0, 100, 100);
    let result =
        YShift::execution(&table, YShiftCustomParameter::Index { z: 25, index: -3 }).unwrap();
...
#[test]
fn y_shift_below_zero_is_error() {
    // y=0 を下方向へ動かすと範囲外。
    let table = table_with(25, 0, 100, 0);
    let result = YShift::execution(&table, YShiftCustomParameter::Index { z: 25, index: -1 });

    assert!(result.is_err());
}

#[test]
fn y_shift_above_max_is_error() {
    // z=2 の最北セル y=3 を上へ動かすと範囲外。
    let table = table_with(2, 0, 0, 3);
    let result = YShift::execution(&table, YShiftCustomParameter::Index { z: 2, index: 1 });
