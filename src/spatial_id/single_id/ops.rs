...
impl SingleId {
    ...
    /// 時間的な差分は [`TemporalId`](crate::TemporalId) の差集合（絶対秒区間ベース、最大2区間）の結果個数に比例する。
    ///
    /// # 動作例
    ///
    /// 重なりがない場合:
    ...
    /// 時間軸については [`TemporalId`](crate::TemporalId) の交差（絶対秒区間ベース）で重なりを求める。
    /// 重なりがない場合は `None` を返す。
    ///
    /// # パラメーター
    ...
    /// 時間軸の判定は [`TemporalId`](crate::TemporalId) の交差計算の計算量に従う。
    ///
    /// # 動作例
    ///
    /// 祖先と子孫の重なり:
