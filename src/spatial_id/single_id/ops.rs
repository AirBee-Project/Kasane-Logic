...
impl SingleId {
    ...
    /// 時間的な差分は [`crate::TemporalSegment::difference`] の結果個数に比例する。
    ///
    /// # 動作例
    ///
    /// 重なりがない場合:
    ...
    /// 時間軸については [`crate::TemporalSegment::intersection`] を用いて重なりを求める。
    /// 重なりがない場合は `None` を返す。
    ///
    /// # パラメーター
    ...
    /// 時間軸の判定は [`crate::TemporalSegment::intersection`] の計算量に従う。
    ///
    /// # 動作例
    ///
    /// 祖先と子孫の重なり:
