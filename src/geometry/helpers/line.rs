...
pub fn line(z: u8, a: Coordinate, b: Coordinate) -> Result<impl Iterator<Item = SingleId>, Error> {
    ...
    // インデックス差分を i64 で計算し、マンハッタン距離を求める
    let diff = ((v1.f() - v2.f()).abs()
        + (v1.x() as i64 - v2.x() as i64).abs()
        + (v1.y() as i64 - v2.y() as i64).abs()) as f64;
...
fn line_dda(z: u8, a: Coordinate, b: Coordinate) -> Result<impl Iterator<Item = SingleId>, Error> {
    ...
    let offsets_int = [offsets[0] as i64, offsets[1] as i64, offsets[2] as i64];
    ...
    let d_o1 = if (vp2[other_flag_1] - vp1[other_flag_1]).abs() > f64::EPSILON {
        d_total[max_flag] / d_total[other_flag_1]
    } else {
        f64::INFINITY
    };
    let d_o2 = if (vp2[other_flag_2] - vp1[other_flag_2]).abs() > f64::EPSILON {
    ...
    let iter = std::iter::once(first).chain((1..=max_steps).map(move |_| {
        ...
        if min_wall == tm_int as f64 {
            tm_int += 1;
            current[0] += sign_i;
        } else if (min_wall - to1).abs() < f64::EPSILON {
            to1 += d_o1;
            current[1] += sign_j;
        } else {
            to2 += d_o2;
