...
impl Triangle {
    ...
    pub fn new(points: [Coordinate; 3], epsilon: f64) -> Result<Self, Error> {
        let ecefs: [Ecef; 3] = points.map(|coord| coord.into());

        if Ecef::is_not_collinear(&ecefs, epsilon) {
            Ok(Self { points: ecefs })
        } else {
            todo!()
        }
    }
